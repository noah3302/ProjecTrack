...
class TaskMapper(Mapper):
    ...
    def find_all(self):
        ...
        for (task_id, tasktitle, description, score, duedate, user_id, phases_id) in tuples:
            task = Task()
            task.set_id(task_id)
    ...
    def find_by_key(self, key):
        ...
        try:
            (task_id, tasktitle, description, score, duedate, user_id, phases_id) = tuples[0]
            task = Task()
            task.set_id(task_id)
            task.set_tasktitle(tasktitle)
            task.set_description(description)
    ...
    def insert(self, task):
        ...
        if count == 0:
            ...
            task.set_id(maxid + 1)
        command = ("INSERT INTO task (task_id, tasktitle, description, score, duedate, user_id, phases_id) VALUES "
                   "(%s, %s, %s, %s, %s, %s, %s)")
        data = (task.get_id(), task.get_tasktitle(), task.get_description(), task.get_score(), task.get_duedate(),
                task.get_user_id(), task.get_phases_id())
    ...
    def update(self, task):
        ...
        command = "UPDATE task SET tasktitle=%s, description=%s, score=%s, duedate=%s, user_id=%s, phases_id=%s WHERE task_id=%s"
